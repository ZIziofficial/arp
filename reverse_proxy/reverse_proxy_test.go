...
func TestNewReverseProxy(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Expected no errors (NewReverseProxy), got %v", err)
	}

	if proxy.ModifyResponse == nil {
		t.Error("Expected ModifyResponse to be set")
	}
	...
	if err != nil {
		t.Errorf("Expected no errors (http.Get), got %v", err)
	}

	if !backendCalled {
		t.Error("Expected a call to the backend, but it didn't happen")
	}
}
...
func TestNewReverseProxy_Director(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Expected no errors (http.NewRequest), got %v", err)
	}
	...
	if req.Header.Get("X-Proxy-UUID") == "" {
		t.Error("Expected Header[X-Proxy-UUID] to be set")
	}

	if req.Header.Get("X-Forwarded-For") != "" {
		t.Errorf("Expected Header[X-Proxy-UUID] to NOT be set, got: %s",
			req.Header.Get("X-Forwarded-For"))
	}

	if req.Host != "www.example.com" {
		t.Errorf("Expected Host to be www.example.com, got: %s", req.Host)
	}
}
